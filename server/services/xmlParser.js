...
class XMLParser {
  ...
  extractReportSummary(parsedXml) {
    return {
      totalAccounts: this.extractNumericValue(parsedXml, ['reportsummary', 'totalaccounts']) ||
                    this.extractNumericValue(parsedXml, ['summary', 'totalaccounts']) ||
                    this.extractNumericValue(parsedXml, ['totalaccounts']) || 0,
      
      activeAccounts: this.extractNumericValue(parsedXml, ['reportsummary', 'activeaccounts']) ||
                     this.extractNumericValue(parsedXml, ['summary', 'activeaccounts']) ||
                     this.extractNumericValue(parsedXml, ['activeaccounts']) || 0,
      
      closedAccounts: this.extractNumericValue(parsedXml, ['reportsummary', 'closedaccounts']) ||
                     this.extractNumericValue(parsedXml, ['summary', 'closedaccounts']) ||
                     this.extractNumericValue(parsedXml, ['closedaccounts']) || 0,
      
      currentBalanceAmount: this.extractNumericValue(parsedXml, ['reportsummary', 'currentbalanceamount']) ||
                           this.extractNumericValue(parsedXml, ['summary', 'currentbalance']) ||
                           this.extractNumericValue(parsedXml, ['currentbalance']) || 0,
      
      securedAccountsAmount: this.extractNumericValue(parsedXml, ['reportsummary', 'securedaccountsamount']) ||
                            this.extractNumericValue(parsedXml, ['summary', 'securedamount']) ||
                            this.extractNumericValue(parsedXml, ['securedamount']) || 0,
      
      unsecuredAccountsAmount: this.extractNumericValue(parsedXml, ['reportsummary', 'unsecuredaccountsamount']) ||
                              this.extractNumericValue(parsedXml, ['summary', 'unsecuredamount']) ||
                              this.extractNumericValue(parsedXml, ['unsecuredamount']) || 0,
      
      last7DaysCreditEnquiries: this.extractNumericValue(parsedXml, ['reportsummary', 'recentenquiries']) ||
                               this.extractNumericValue(parsedXml, ['summary', 'recentenquiries']) ||
                               this.extractNumericValue(parsedXml, ['recentenquiries']) || 0
    };
  }
      ...
      if (accountData) {
        ...
        accountArray.forEach(account => {
          if (account && typeof account === 'object') {
            accounts.push({
              accountNumber: account.accountnumber || account.account_number || account.number || 'N/A',
              accountType: this.mapAccountType(account.type || account.accounttype || 'other'),
              bankName: account.bankname || account.bank_name || account.bank || 'Unknown Bank',
              currentBalance: this.extractNumericValue(account, ['currentbalance']) || 0,
              amountOverdue: this.extractNumericValue(account, ['amountoverdue']) || 0,
              creditLimit: this.extractNumericValue(account, ['creditlimit']) || null,
              accountStatus: this.mapAccountStatus(account.status || 'active'),
              openedDate: this.parseDate(account.openeddate || account.opened_date),
              lastPaymentDate: this.parseDate(account.lastpaymentdate || account.last_payment),
              emiAmount: this.extractNumericValue(account, ['emiamount']) || null
            });
          }
        });
  ...
  mapAccountType(type) {
    ...
    if (typeStr.includes('credit') || typeStr.includes('card')) return 'credit_card';
    if (typeStr.includes('personal')) return 'personal_loan';
    if (typeStr.includes('home') || typeStr.includes('housing')) return 'home_loan';
    if (typeStr.includes('auto') || typeStr.includes('vehicle')) return 'auto_loan';
  ...
  mapAccountStatus(status) {
    ...
    if (statusStr.includes('closed') || statusStr.includes('terminated')) return 'closed';
    if (statusStr.includes('suspended')) return 'suspended';
    if (statusStr.includes('written') || statusStr.includes('off')) return 'written_off';
