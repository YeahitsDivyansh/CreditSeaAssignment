...
import { Eye, Trash2, Calendar, User, CreditCard, TrendingUp, AlertCircle } from 'lucide-react';
import { creditReportsAPI } from '../services/api';
import { useApp } from '../context/AppContext';
import toast from 'react-hot-toast';

const ReportsList = () => {
  const { reports, pagination, loading, setReports, setLoading, setError, deleteReport } = useApp();
  ...
  const handleDelete = async (reportId, pan) => {
    if (!window.confirm(`Are you sure you want to delete the credit report for PAN: ${pan}?`)) {
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {pagination.totalCount} report{pagination.totalCount !== 1 ? 's' : ''} found
          </p>
        </div>
      ...
      {reports.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">Upload an XML file to get started</p>
          <Link
            to="/upload"
            ...
            <div key={report._id} className="bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow">
              <div className="p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className={`px-2 py-1 rounded-full text-xs font-medium ${getCreditScoreColor(report.creditScore)}`}>
                    {report.creditScore}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-600">{getCreditScoreLabel(report.creditScore)}</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        report.creditScore >= 750 ? 'bg-green-500' :
                        report.creditScore >= 650 ? 'bg-yellow-500' :
                        report.creditScore >= 550 ? 'bg-orange-500' : 'bg-red-500'
                      }`}
